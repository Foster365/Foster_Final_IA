...
public class PatrolState : State
{
    ...
    public override void ExecuteState(EntityModel model)
    {
        ...
        var modelRef = _movementDatas[model];
        Debug.Log("Leader patrol state");
        if (_movementDatas[model].patrolCount <= timer)
        {
            ...
            {
                var finalPath = modelRef.model.Controller.CharAIController.AStarPathFinding.finalPath;
                //Acá corro Astar.
                ...
                {
                    var dirToNextPoint = (finalPath[_movementDatas[model].patrolCount].worldPosition - model.transform.position).normalized;
